...
function define_energy_system_fixed_investment(pv, wind, demand, heatdemand; p = default_es_pars, inv_dict, strict_flex=false,)
    number_of_hours = minimum([length(pv), length(demand), length(wind)])
    c_sto_op = p[:c_sto_op]
    c_i = p[:c_i]
    c_o = p[:c_o]
    ...
    energy_system = @stochastic_model begin 
        @stage 1 begin
            @parameters begin
                ...
                u_wind = u_wind
                u_storage = u_storage
                u_heatpump = u_heatpump
                u_heat_storage = u_heat_storage
                # Costs in Euro/1kWp
                c_sto_op = c_sto_op
                c_i = c_i
                c_o = c_o
            ...
            @constraint(model, [t in 1:number_of_hours], -heatdemand[t] - heat_sto_in[t] + heat_sto_out[t] + COP*heatpumpflow[t] - heat_losses*heat_sto_soc[t] == 0)
            # Investment costs
            @objective(model, Min, c_i * sum(gci) - c_o * sum(gco) + c_sto_op * sum(sto_in) + c_sto_op * sum(sto_out))
        end
        @stage 2 begin
            @parameters begin
                # Investment parameters
