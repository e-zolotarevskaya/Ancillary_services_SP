...
## 
#Validation Test for the system!




#global parameters for units 

u_pv_opt = value.(sp[1, :u_pv])
u_wind_opt = value.(sp[1, :u_wind])
u_storage_opt = value.(sp[1, :u_storage])
#print(od)
...
validation_model = @stochastic_model begin 
    @stage 1 begin
        ...
        # Storage model
        @decision(model, storage[t in timesteps])
        @constraint(model, [t in timesteps], -0.5*u_storage_opt*storage_scale <= sum(storage[t_s:t]))
        @constraint(model, [t in timesteps], sum(storage[t_s:t]) <= 0.5*u_storage_opt*storage_scale)
        @constraint(model, sum(storage) == 0)
        # Energy balance
        @constraint(model, [t in timesteps], 
    ...
    @stage 2 begin
        ...
        # The storage and other fast acting components use the recourse variables here.
        # They provide the balance. Grid connection is not allowed, as we are suporting the grid here. 
        @constraint(model, gci_opt[t_xi]-gco_opt[t_xi]+u_pv_opt*pv[t_xi]+u_wind_opt*wind[t_xi]-demand[t_xi]+sto2[t_xi]+F*s_xi==0)
        # Post event energy balance
        @constraint(model, [t in (t_xi+1):t_f],
...
#sp_rs : resampeled model 
sp_rs = instantiate(validation_model, s, 5, optimizer = GLPK.Optimizer)
##
optimize!(sp_rs)

od_rs = optimal_decision(sp_rs)
objective_value(sp_rs)
##
print(od_rs)

##
# Main result
println("Termination status: $(termination_status(sp_rs))")
